# MYCELIUM.md — Living Repository Protocol
...
2. **For targeted lookup:** `python3 "$(cat .mycelium/plugin-root)/skills/core/scripts/recall_lessons.py" --living-dir .living/ --tag <tag>` — fetches only the matching entries instead of pulling the whole file. Also accepts `--id L-42`, `--since YYYY-MM-DD`.
...
## Bundled Mycelium Resources

Mycelium scripts, references, and templates remain in the installed plugin,
not in this project. `.mycelium/plugin-root` records the current machine-local
plugin root. Resolve any path beginning with `skills/` or `network/` against
that directory. Trusted Codex plugin hooks refresh the pointer automatically
after an upgrade. For Claude Code, or for a repository created before the
provider-neutral pointer existed, re-run the idempotent Mycelium migration if
the installed bundle path changes.
...
- **bioinformatics** — See `.living/conventions/bioinformatics/analysis-conventions.md` Install with mycelium's `install-convention` mode (e.g., "install bioinformatics conventions").
...
2. **Drill in selectively** — for entries that look relevant, either:
   - Fetch just the entries: `python3 "$(cat .mycelium/plugin-root)/skills/core/scripts/recall_lessons.py" --living-dir .living/ --tag <tag>` or `--id L-42`
...
- Use marimo for exploration (start from `$(cat .mycelium/plugin-root)/skills/core/templates/marimo-notebook-header.py`), Python scripts for reproducible pipelines
...
5. **Log findings**: If the work produced a scientific finding (empirical observation, validated/invalidated hypothesis, quantitative result, or domain discovery), crystallize it to `.living/findings/{topic}.md`. Check existing topics first for consistency. Prefer broad topic names. Resolve the bundled `skills/core/templates/findings-entry.md` template through `.mycelium/plugin-root`, then update `.living/findings/FINDINGS_REGISTRY.md` with `python3 "$(cat .mycelium/plugin-root)/skills/core/scripts/upsert_table_row.py"`.
6. **Log todos**: If future work is identified, create a detailed `todo/[item].md` writeup as needed, then update `todo/TODO_REGISTRY.md` with `python3 "$(cat .mycelium/plugin-root)/skills/core/scripts/upsert_table_row.py" todo/TODO_REGISTRY.md '<item title>' '| <item title> | ... |'`
7. **Validate structure**: Run `python3 "$(cat .mycelium/plugin-root)/skills/core/scripts/validate_structure.py" --target-dir .`
...
10. **Session summary**: Write the complete five-section handoff to the exact path supplied by the current lifecycle context. Identified roots use `.mycelium/run/<host>/<session-id>/last-session.md`; legacy identity-free hosts use `.mycelium/last-session.md`

### Automated Enforcement

Mycelium installs six repository-local Claude Code hooks during `init`. Codex
uses five plugin-bundled Codex command hooks, so initialization does not create
or depend on a project-local `.codex/hooks.json`:

| Hook | Event | What it does |
|------|-------|--------------|
| `mycelium-health.sh` | SessionStart | Checks `.living/` health, records session timestamp |
| `mycelium-post-action.sh` | PostToolUse (shell) | Detects code execution and directs the active agent to run the full post-action protocol (debounced: one directive per work cycle) |
| `mycelium-data-tracker.sh` | PostToolUse (shell) | Captures execution-time data-lineage events for analysis commands |
| `mycelium-activity-tracker.sh` | PostToolUse (edits) | Records edited paths for session-local change accounting |
| `mycelium-read-tracker.sh` | PostToolUse (reads; Claude only) | Records access to living-memory files; Codex does not expose internal reads |
| `mycelium-stop-check.sh` | Stop | Serializes lineage consolidation before lifecycle enforcement, then blocks only if significant work was not reflected into `.living/` |
...
Codex requires a one-time approval for each exact plugin command hook. After
installing or upgrading Mycelium, open `/hooks` in a current Codex CLI—not the
desktop app—review and trust all five Mycelium hooks, fully exit Codex, then
start a fresh task. If `/hooks` is absent, run `codex update` and relaunch the
CLI first. Until approval, Codex intentionally skips the plugin-bundled
commands.
