...
- **Key inputs**: BFD gene predictions/`.gbk` (`/bigdata/stajichlab/shared/projects/BFD/Fungi_BFD_runs/genome_annotation/<out>/`); local fallback runs under `analysis/genome_bioactivity_linkage/results/{pfam_hmmscan,signalp,predgpi,deeptmhmm,rbh,antismash_ncbi}/<species>/` (`run_pfam.sh`, `run_signalp.sh`, `run_predgpi.sh`, `run_deeptmhmm.sh`, `run_rbh.sh`, `run_antismash_reference.sh`); `analysis/sirius_annotation/sirius_annotations.tsv`; `analysis/differential_features_primary/all_significant_features_summary.tsv`; `data/metdata/curated_gnps_metadata.tsv`.
