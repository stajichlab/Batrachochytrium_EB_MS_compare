...
- **Follow-ups resolved 2026-08-26**: candidate tables now carry RNA-seq expression columns (`rna_is_expressed` etc., via `analysis/rnaseq_expression/`); tier-1 NRPS BGC characterization at `results/TIER1_NRPS_CHARACTERIZATION.md`; Bsal protein-count/duplication diagnostics at `results/duplication_check/`; `parse_pfam_domains.py` domtblout offsets spot-checked against real files (correct).
