...
def plot_all(axes, meta, prop, out_path):
    ...
    ax.set_title(f"Bray-Curtis PCoA -- all fungal analysis samples (n={len(meta)})")
...
def plot_condition(axes, meta, prop, out_path):
    ...
    ax.set_xlabel(f"PCoA1 ({prop[0]:.1%})")
    ax.set_ylabel(f"PCoA2 ({prop[1]:.1%})")
    ax.set_title(f"Bray-Curtis PCoA -- sampled condition (n={len(meta)})")
...
def plot_stage_group(axes, meta, prop, out_path):
    ...
    ax.set_xlabel(f"PCoA1 ({prop[0]:.1%})")
    ax.set_ylabel(f"PCoA2 ({prop[1]:.1%})")
    ax.set_title(f"Bray-Curtis PCoA -- Zoospore vs Developed (n={len(meta)})")
    ax.legend(frameon=False, fontsize=8, loc="center left", bbox_to_anchor=(1.0, 0.5))
    ax.set_aspect("equal", adjustable="datalim")
    ax.spines[["top", "right"]].set_visible(False)
    fig.tight_layout()
